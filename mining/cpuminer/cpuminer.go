...
type Config struct {
	...
	ProcessBlock func(*bronutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *bronutil.Block) bool {
	...
	log.Infof("Block submitted via CPU miner accepted (hash %s, "+
		"amount %v)", block.Hash(), bronutil.Amount(coinbaseTx.Value))
	return true
}
...
out:
	for {
		...
		if m.solveBlock(template.Block, curHeight+1, ticker, quit) {
			block := bronutil.NewBlock(template.Block)
			m.submitBlock(block)
		}
	}
