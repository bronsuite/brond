...
const (
	...
	MinHighPriority = bronutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr bronutil.Address) (*bronutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *bronutil.Tx, height int32) error {
...
func logSkippedDeps(tx *bronutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress bronutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*bronutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := bronutil.NewTx(coinbaseTx.MsgTx().Copy())
...
func AddWitnessCommitment(coinbaseTx *bronutil.Tx,
	blockTxns []*bronutil.Tx) []byte {
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A bronutil.Block should use saved in the state to avoid
