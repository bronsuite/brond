...
import (
	...
	"github.com/bronsuite/brond/btcec/v2"
...
func (s *fakeChain) FetchUtxoView(tx *bronutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *bronutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *bronutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   bronutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*bronutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee bronutil.Amount,
	signalsReplacement bool) (*bronutil.Tx, error) {

	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput bronutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*bronutil.Tx, error) {
	txChain := make([]*bronutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, bronutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := bronutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *bronutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee bronutil.Amount,
	signalsReplacement, confirmed bool) *bronutil.Tx {
...
func testPoolMembership(tc *testContext, tx *bronutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   bronutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *bronutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*bronutil.Tx, []*bronutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*bronutil.Tx, []*bronutil.Tx) {
				...
				return tx, []*bronutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*bronutil.Tx, []*bronutil.Tx) {
				...
				return tx, []*bronutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = bronutil.SatoshiPerBitcoin
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(ctx *testContext) (*bronutil.Tx, []*bronutil.Tx) {
				...
				return childReplace, []*bronutil.Tx{childNoReplace}
			},
			err: "",
		},
	}
