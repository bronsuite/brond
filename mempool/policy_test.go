...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee bronutil.Amount // minimum relay transaction fee.
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: bronutil.MaxSatoshi, PkScript: pkScript},
			bronutil.MaxSatoshi,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	addr, err := bronutil.NewAddressPubKeyHash(addrHash[:],
	...
	for _, test := range tests {
		...
		err := checkTransactionStandard(bronutil.NewTx(&test.tx),
