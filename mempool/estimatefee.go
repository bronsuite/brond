...
// the given fee rate.
func (rate SatoshiPerByte) Fee(size uint32) bronutil.Amount {
	// If our rate is the error value, return that.
	if rate == SatoshiPerByte(-1) {
		return bronutil.Amount(-1)
	}

	return bronutil.Amount(float64(rate) * float64(size))
}

// NewSatoshiPerByte creates a SatoshiPerByte from an Amount and a
// size in bytes.
func NewSatoshiPerByte(fee bronutil.Amount, size uint32) SatoshiPerByte {
...
func (ef *FeeEstimator) ObserveTransaction(t *TxDesc) {
	...
	if _, ok := ef.observed[hash]; !ok {
		...
		ef.observed[hash] = &observedTransaction{
			hash:     hash,
			feeRate:  NewSatoshiPerByte(bronutil.Amount(t.Fee), size),
			observed: t.Height,
...
func (ef *FeeEstimator) RegisterBlock(block *bronutil.Block) error {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	transactions := make(map[*bronutil.Tx]struct{})
	...
	for t := range transactions {
		...
		// Refer to github.com/bronsuite/brond/issues/1660 for more information.
		//
