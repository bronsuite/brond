...
const (
	...
	DefaultMinRelayTxFee = bronutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee bronutil.Amount) int64 {
	...
	if minFee < 0 || minFee > bronutil.MaxSatoshi {
		minFee = bronutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *bronutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func IsDust(txOut *wire.TxOut, minRelayTxFee bronutil.Amount) bool {
...
func checkTransactionStandard(tx *bronutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee bronutil.Amount,
...
func GetTxVirtualSize(tx *bronutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
