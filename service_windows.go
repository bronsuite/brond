...
const (
	// svcName is the name of brond service.
	svcName = "brondsvc"
...
// logServiceStartOfDay logs information about brond when the main server has
...
// brondService houses the main service handler which handles all service
// updates and launching brondMain.
type brondService struct{}
...
// long-running brondMain (which is the real meat of brond), handles service
// change requests, and notifies the service control manager of changes.
func (s *brondService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start brondMain in a separate goroutine so the service can start
...
// installService attempts to install the brond service.  Typically this should
...
// removeService attempts to uninstall the brond service.  Typically this should
...
// startService attempts to start the brond service.
...
func serviceMain() (bool, error) {
	...
	// needed to run brond on Windows in CI environments like Travis.
	...
	err = svc.Run(svcName, &brondService{})
	if err != nil {
