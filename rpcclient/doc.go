...
Bitcoin RPC server that uses a brond/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with brond (https://github.com/bronsuite/brond),
btcwallet (https://github.com/btcsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, brond and
...
brond or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by brond and
...
All notifications provided by brond require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by brond (and
...
Also, it is important to realize that brond intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the brond RPC server directly, only the RPCs which are related to
