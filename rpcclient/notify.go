...
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*bronutil.Tx, error) {
	...
	transactions := make([]*bronutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = bronutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*bronutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return bronutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := bronutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance bronutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := bronutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
