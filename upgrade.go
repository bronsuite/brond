...
// oldbrondHomeDir returns the OS specific home directory brond used prior to
...
func oldbrondHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "brond")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".brond")
	}
...
// location prior to brond version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to brond
...
func upgradeDBPaths() error {
	...
	oldDbRoot := filepath.Join(oldbrondHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "brond.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "brond_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "brond_regtest.db"), "regtest")
