...
unit:
	...
	cd bronutil; $(GOTEST_DEV) ./... -test.timeout=20m
	cd bronutil/psbt; $(GOTEST_DEV) ./... -test.timeout=20m
...
unit-cover: $(GOACC_BIN)
	...
	cd bronutil; $(GOACC_BIN) ./...

	cd bronutil/psbt; $(GOACC_BIN) ./...
...
unit-race:
	...
	cd bronutil; env CGO_ENABLED=1 GORACE="history_size=7 halt_on_errors=1" $(GOTEST) -race -test.timeout=20m ./...
	cd bronutil/psbt; env CGO_ENABLED=1 GORACE="history_size=7 halt_on_errors=1" $(GOTEST) -race -test.timeout=20m ./...
...
clean:
	...
	$(RM) coverage.txt btcec/coverage.txt bronutil/coverage.txt bronutil/psbt/coverage.txt
