...
func addrToKey(addr bronutil.Address) ([addrKeySize]byte, error) {
	...
	case *bronutil.AddressWitnessScriptHash:
		...
		// push to 20-bytes.
		copy(result[1:], bronutil.Hash160(addr.ScriptAddress()))
		return result, nil
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[chainhash.Hash]*bronutil.Tx
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *bronutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *bronutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *bronutil.Block,
	stxos []blockchain.SpentTxOut) error {
...
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr bronutil.Address,
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *bronutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[chainhash.Hash]*bronutil.Tx)
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) AddUnconfirmedTx(tx *bronutil.Tx, utxoView *blockchain.UtxoViewpoint) {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr bronutil.Address) []*bronutil.Tx {
	// Ignore unsupported address types.
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*bronutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[chainhash.Hash]*bronutil.Tx),
