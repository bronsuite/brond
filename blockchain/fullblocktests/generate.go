...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: bronutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := bronutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*bronutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, bronutil.NewTx(tx))
	}
...
func additionalCoinbase(amount bronutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee bronutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee bronutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee bronutil.Amount) *wire.MsgTx {
