...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *bronutil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CalcSequenceLock(tx *bronutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
	b.chainLock.Lock()
...
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *bronutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *bronutil.Block,
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *bronutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *bronutil.Block) int {
...
func (b *BlockChain) reorganizeChain(detachNodes, attachNodes *list.List) error {
	...
	detachBlocks := make([]*bronutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]SpentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*bronutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *bronutil.Block, flags BehaviorFlags) (bool, error) {
...
type IndexManager interface {
	...
	// required.
	ConnectBlock(database.Tx, *bronutil.Block, []SpentTxOut) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain. The set of outputs scripts that were spent within
	// this block is also returned so indexers can clean up the prior index
	// state for this block.
	DisconnectBlock(database.Tx, *bronutil.Block, []SpentTxOut) error
}
