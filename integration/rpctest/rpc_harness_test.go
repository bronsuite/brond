...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt bronutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(bronutil.Amount(5 * bronutil.SatoshiPerBitcoin))
	...
	txid = genSpend(bronutil.Amount(500 * bronutil.SatoshiPerBitcoin))
	blockHashes, err = r.Client.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(bronutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*bronutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, bronutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := bronutil.Amount(250 * bronutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = bronutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := bronutil.Amount(50 * bronutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := bronutil.Amount(numMatureOutputs * 50 * bronutil.SatoshiPerBitcoin)
