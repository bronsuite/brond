...
func makeTestOutput(r *rpctest.Harness, t *testing.T,
	amt bronutil.Amount) (*btcec.PrivateKey, *wire.OutPoint, []byte, error) {
	...
	// spend.
	a, err := bronutil.NewAddressPubKey(key.PubKey().SerializeCompressed(), r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0113Activation(t *testing.T) {
	...
	const outputValue = bronutil.SatoshiPerBitcoin
	...
	txns := []*bronutil.Tx{bronutil.NewTx(tx)}
...
func createCSVOutput(r *rpctest.Harness, t *testing.T,
	numSatoshis bronutil.Amount, timeLock int32,
	...
	p2shAddr, err := bronutil.NewAddressScriptHash(csvScript, r.ActiveNet)
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBIP0068AndBIP0112Activation(t *testing.T) {
	...
	const (
		outputAmt         = bronutil.SatoshiPerBitcoin
	...
	for txVersion := int32(0); txVersion < 3; txVersion++ {
		...
		txns := []*bronutil.Tx{bronutil.NewTx(spendingTx)}
		block, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
		if err != nil {
			t.Fatalf("unable to submit block: %v", err)
		}
	...
	for i, test := range tests {
		...
		if !test.accept {
			txns := []*bronutil.Tx{bronutil.NewTx(test.tx)}
			_, err := r.GenerateAndSubmitBlock(txns, -1, time.Time{})
			if err == nil {
