...
import (
	...
	"github.com/bronsuite/brond/btcec/v2/schnorr"
	"github.com/bronsuite/brond/bronutil"

	"github.com/bronsuite/brond/btcec/v2/ecdsa"
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []bronutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []bronutil.Address, int, error) {
...
// versions.
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []bronutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		for _, addr := range addresses {
			...
			// bad input.
			pkaddr := addr.(*bronutil.AddressPubKey)
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []bronutil.Address,
...
type KeyDB interface {
	GetKey(bronutil.Address) (*btcec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(bronutil.Address) (*btcec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address bronutil.Address) (*btcec.PrivateKey, bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(bronutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(bronutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address bronutil.Address) ([]byte, error) {
	return sc(address)
}
