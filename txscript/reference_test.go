...
func testScripts(t *testing.T, tests [][]interface{}, useSigCache bool) {
	...
	for i, test := range tests {
		...
		if witnessData, ok := test[0].([]interface{}); ok {
			...
			inputAmt, err = bronutil.NewAmount(witnessData[len(witnessData)-1].(float64))
			if err != nil {
...
testloop:
	for i, test := range tests {
		...
		tx, err := bronutil.NewTxFromBytes(serializedTx)
		if err != nil {
...
func executeTaprootRefTest(t *testing.T, testCase taprootJsonTest) {
	...
	tx, err := bronutil.NewTxFromBytes(txHex)
	if err != nil {
		t.Fatalf("unable to decode hex: %v", err)
	}
