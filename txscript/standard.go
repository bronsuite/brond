...
// specified address.
func PayToAddrScript(addr bronutil.Address) ([]byte, error) {
...
func MultiSigScript(pubkeys []*bronutil.AddressPubKey, nrequired int) ([]byte, error) {
...
func pubKeyHashToAddrs(hash []byte, params *chaincfg.Params) []bronutil.Address {
	// Skip the pubkey hash if it's invalid for some reason.
	var addrs []bronutil.Address
	addr, err := bronutil.NewAddressPubKeyHash(hash, params)
	if err == nil {
...
func scriptHashToAddrs(hash []byte, params *chaincfg.Params) []bronutil.Address {
	// Skip the hash if it's invalid for some reason.
	var addrs []bronutil.Address
	addr, err := bronutil.NewAddressScriptHashFromHash(hash, params)
	if err == nil {
		addrs = append(addrs, addr)
	}
	return addrs
...
func ExtractPkScriptAddrs(pkScript []byte,
	chainParams *chaincfg.Params) (ScriptClass, []bronutil.Address, int, error) {
	...
	if data := extractPubKey(pkScript); data != nil {
		var addrs []bronutil.Address
		addr, err := bronutil.NewAddressPubKey(data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if details.valid {
		...
		addrs := make([]bronutil.Address, 0, len(details.pubKeys))
		for _, pubkey := range details.pubKeys {
			addr, err := bronutil.NewAddressPubKey(pubkey, chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
		}
	...
	if hash := extractWitnessPubKeyHash(pkScript); hash != nil {
		var addrs []bronutil.Address
		addr, err := bronutil.NewAddressWitnessPubKeyHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if hash := extractWitnessV0ScriptHash(pkScript); hash != nil {
		var addrs []bronutil.Address
		addr, err := bronutil.NewAddressWitnessScriptHash(hash, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	if rawKey := extractWitnessV1KeyBytes(pkScript); rawKey != nil {
		var addrs []bronutil.Address
		addr, err := bronutil.NewAddressTaproot(rawKey, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
