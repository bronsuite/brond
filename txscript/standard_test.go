...
// newAddressPubKey returns a new bronutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) bronutil.Address {
	addr, err := bronutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new bronutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) bronutil.Address {
	addr, err := bronutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new bronutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) bronutil.Address {
	addr, err := bronutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// newAddressTaproot returns a new bronutil.AddressTaproot from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressTaproot(scriptHash []byte) bronutil.Address {
	addr, err := bronutil.NewAddressTaproot(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
		panic("invalid script hash in test source")
	}
...
func TestExtractPkScriptAddrs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			addrs: []bronutil.Address{newAddressTaproot(
...
// bogusAddress implements the bronutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the bronutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := bronutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := bronutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := bronutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := bronutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := bronutil.NewAddressPubKey(hexToBytes("0411"+
	...
	p2wsh, err := bronutil.NewAddressWitnessScriptHash(hexToBytes("e981bd992a43650657"+
	...
	p2tr, err := bronutil.NewAddressTaproot(hexToBytes("3a8e170b546c3b122ab9c175e"+
	...
	p2wpkh, err := bronutil.NewAddressWitnessPubKeyHash(hexToBytes("748e50366adb8"+
	...
	tests := []struct {
		...
		{(*bronutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*bronutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*bronutil.AddressPubKey)(nil), "", errUnsupportedAddress},
		{(*bronutil.AddressWitnessPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*bronutil.AddressWitnessScriptHash)(nil), "", errUnsupportedAddress},
		{(*bronutil.AddressTaproot)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*bronutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*bronutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
