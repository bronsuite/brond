...
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a bronutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "12gpXQVcCL2qhTNQgyLVdCFG2Qs2px98nV"
	address, err := bronutil.DecodeAddress(addressStr, &chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	addr, err := bronutil.NewAddressPubKeyHash(pubKeyHash,
	...
	lookupKey := func(a bronutil.Address) (*btcec.PrivateKey, bool, error) {
...
func ExampleScriptTokenizer() {
	...
	// some other source.
	hash160 := bronutil.Hash160([]byte("example"))
